...
class GestureClassifier(context: Context) {
    ...
    init {
        ...
        interpreterDynamic = Interpreter(loadModelFile(context, "model_dynamic_m.tflite"))
        ...
        println("Static labels count = ${staticLabels.size}")
        println("Dynamic labels count = ${dynamicLabels.size}")
    }

    val numClassesStatic = staticLabels.size
    val numClassesDynamic = dynamicLabels.size
    ...
    fun preProcessPointHistory(
        ...
        for (frame in floatArrayList) {
            for (i in frame.indices step 2) {
                val x = (frame[i] - baseX) / imageWidth
                val y = (frame[i + 1] - baseY) / imageHeight
                flatPoints.add(x)
                flatPoints.add(y)
            }
        }
...
class LandmarkHistoryBuffer(val maxFrames: Int = 16, val numPoints: Int = 6) {
    ...
    fun denormalizePoints(
        ...
        require(normalizedPoints.size == 12) { "Tablica powinna mieć dokładnie 12 elementów (6 punktów x,y)" }

        val denormalized = FloatArray(12)
        for (i in 0 until 6) {
            denormalized[i * 2] = normalizedPoints[i * 2] * width      // x denormalizowane
            denormalized[i * 2 + 1] = normalizedPoints[i * 2 + 1] * height // y denormalizowane
        }
        return denormalized
    }
    ...
    // Dodajemy poniżej, jeśli potrzebujesz używać size i toList
    val size: Int
    get() = buffer.size
